...
export default function StepTwo({
  ...
  async function createDemandPlan(year: number, deptId: number, file: FormData) {
    // setLoading(true);
    try {
      const loginres = await DemandPlanServices.createDemandPlan("Materials", templateType, year, deptId, file);
      console.log(loginres.data);
      ...
      toast({
        status: "success",
        description: "Demand plan created!",
        position: "bottom-right",
      });
      setLoading(false);
      ...
      toast({
        status: "error",
        description: "Please review the errors above and correct them before submitting.",
        position: "bottom-right",
      });
      // setApiErrorMessage(errorMessage, "error");
      // return;
  ...
  const handleMouseOver = (event: MouseEvent) => {
    ...
    if (target.classList.contains('e-cell')) {
      ...
      // console.log(`Hovered Cell Value: ${cellValue}`);
  ...
  const handleCellEdit = (args: CellEditEventArgs) => {
    ...
    if (args.address.includes('D') && args.address !== "Sheet1!D1") {
      setIsMesc(true);
    } else {
      setIsMesc(false);
    }
  ...
  const handleCellSave = (args: any) => {
    ...
    if (spreadsheet && args.address.includes('D')) {
  ...
  const onFetchFailure = (args:any) => {
    console.error('Fetch failed:', args);
};
  return (
