...
const Plans = () => {
  ...
  const [selectedDpType, setSelectedDpType] = useState<string>();
  ...
  async function getInreviewPlan(deptId: number) {
    setLoading(true);
    try {
      const res = await DemandPlanServices.getDemandPlan("Draft", deptId);
  ...
  async function getErrorsById(dpId: number) {
    setLoading(true);
    try {
      const res = await DemandPlanServices.getErrorsByid(dpId);
      setErrors(res?.data);
      toast({
        status: "success",
        description: "Welcome",
  ...
  async function getPlanTemplate(dpId: number, download: boolean) {
    setLoading(true);
    try {
      const res = await DemandPlanServices.getInvalidPlanTemplate(dpId, download);
      console.log(res)
      if (res) {
        setFile(res)
        onOpen();
  ...
  const handleView = (id: number, type: string) => {
    ...
    getErrorsById(id)
    setSelectedDpType(type)
  }
  useEffect(() => {
    const { id, type } = router.query
    if (id && type) {
      setSelectedDpType(String(type));
      getPlanTemplate(Number(id), false);
      getErrorsById(Number(id))
  ...
  return (
    <>
                ...
                <tr key={index} className="border-b">
                  <td className="ps-4 pe-9 py-2 flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#757575]">last updated 5 days ago</p>
                    </div>
                  </td>
                  <td className="px-4 py-2 text-xs"></td>
                  <td className="px-4 py-2 text-xs">{item.demandplan?.year}</td>
                  <td className="py-2 text-xs">
                    <span className="bg-[#FFDEE3] font-medium uppercase text-[#E60000] py-2 px-3 rounded-full text-[10px]">
                      {!item.validationStatus ? "Validation error" : "Proccess Stopped"}
                    </span>
                  </td>
                  <td className="flex items-center py-2 space-x-4">
                    <button className="text-[#0085C8] bg-[#E3F1F9] py-2 px-3 rounded-full flex items-center" onClick={() => handleView(item.demandplan?.id, item.type)} >
                    ...
                    <svg className="cursor-pointer" onClick={() => getPlanTemplate(item.demandplan.id, true)} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <ViewDemandPlan isActivity={selectedDpType === "Activities"} errors={errors || []} file={file} cycleYear={2025} isOpen={isOpen} onClose={() => onClose()} />
    </>
  );
};
